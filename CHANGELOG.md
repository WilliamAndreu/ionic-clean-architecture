...
- **`core-interface/` renamed to `interfaces/`** — folder moved from `src/core/core-interface/` to `src/core/interfaces/`; path alias updated in `tsconfig.json` from `@interface-core/*` to `@interfaces/*`; all 15 affected `.ts` files updated
...
- **Infinite scroll spinner** — `ion-infinite-scroll-content` was rendering both a spinner and the three-dot loading text simultaneously; `loadingSpinner="null"` disables the spinner, leaving only the three dots
